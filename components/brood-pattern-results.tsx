...
export default function BroodPatternResults({ results }: BroodPatternResultsProps) {
  ...
  useEffect(() => {
    if (results && !hasAutoSaved.current) {
      // Map hiveHealthScore to healthScore for storage compatibility
      const mappedResults = {
        ...results,
        healthScore: results.hiveHealthScore,
        healthStatus: results.healthStatus
      }
      const resultId = `${results.totalDetections}_${results.hiveHealthScore}_${JSON.stringify(results.counts)}`
      if (savedResultId.current === resultId) return

      const timer = setTimeout(() => {
        hasAutoSaved.current = true
        savedResultId.current = resultId
        saveBroodAnalysis(mappedResults)
